...
backup() {
  ...
  if aws s3 "$AWS_ARGS" rm s3://"${S3_BUCKET}"/${S3_KEY_PREFIX}latest.tgz; then
    echo "Removed latest backup from S3"
  else
    echo "No latest backup exists in S3"
  fi
  if aws s3 "$AWS_ARGS" cp "$BACKUP_ARCHIVE_PATH" s3://"${S3_BUCKET}"/${S3_KEY_PREFIX}latest.tgz; then
    echo "Backup file copied to s3://${S3_BUCKET}/${S3_KEY_PREFIX}latest.tgz"
  else
    echo "Backup file failed to upload"
    exit 1
  fi
  if aws s3api "$AWS_ARGS" copy-object --copy-source "${S3_BUCKET}"/${S3_KEY_PREFIX}latest.tgz --key ${S3_KEY_PREFIX}"${DATETIME}".tgz --bucket "$S3_BUCKET"; then
